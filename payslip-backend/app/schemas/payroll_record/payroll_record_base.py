...
class PayrollRecordBase(BaseModel):
    ...
    def validate_dates(self):
        if self.run_date and (self.run_date.year != self.payroll_year or self.run_date.month != self.payroll_month):
            raise ValueError("run_date must correspond to the specified payroll_year and payroll_month")
        return self
